...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagsubmit)
    {
        info->event[info->num] = EVENT_submit;
        info->num++;
        dataPoints->valuesubmit = gizStandardDecompressionValue(submit_BYTEOFFSET,submit_BITOFFSET,submit_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagignition)
    {
        info->event[info->num] = EVENT_ignition;
        info->num++;
        dataPoints->valueignition = gizStandardDecompressionValue(ignition_BYTEOFFSET,ignition_BITOFFSET,ignition_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagmode_Slow_fire_old_soup)
    {
        info->event[info->num] = EVENT_mode_Slow_fire_old_soup;
        info->num++;
        dataPoints->valuemode_Slow_fire_old_soup = gizStandardDecompressionValue(mode_Slow_fire_old_soup_BYTEOFFSET,mode_Slow_fire_old_soup_BITOFFSET,mode_Slow_fire_old_soup_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagmode_Firewood)
    {
        info->event[info->num] = EVENT_mode_Firewood;
        info->num++;
        dataPoints->valuemode_Firewood = gizStandardDecompressionValue(mode_Firewood_BYTEOFFSET,mode_Firewood_BITOFFSET,mode_Firewood_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagmode_Steamed_bag)
    {
        info->event[info->num] = EVENT_mode_Steamed_bag;
        info->num++;
        dataPoints->valuemode_Steamed_bag = gizStandardDecompressionValue(mode_Steamed_bag_BYTEOFFSET,mode_Steamed_bag_BITOFFSET,mode_Steamed_bag_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagdianji_one)
    {
        info->event[info->num] = EVENT_dianji_one;
        info->num++;
        dataPoints->valuedianji_one = gizX2Y(dianji_one_RATIO,  dianji_one_ADDITION, issuedData->attrVals.valuedianji_one); 
    }
        
    if(0x01 == issuedData->attrFlags.flagset_the_time)
    {
        info->event[info->num] = EVENT_set_the_time;
        info->num++;
        dataPoints->valueset_the_time = gizX2Y(set_the_time_RATIO,  set_the_time_ADDITION, issuedData->attrVals.valueset_the_time); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuesubmit != cur->valuesubmit)
    {
        GIZWITS_LOG("valuesubmit Changed\n");
        ret = 1;
    }
    if(last->valueignition != cur->valueignition)
    {
        GIZWITS_LOG("valueignition Changed\n");
        ret = 1;
    }
    if(last->valuemode_Slow_fire_old_soup != cur->valuemode_Slow_fire_old_soup)
    {
        GIZWITS_LOG("valuemode_Slow_fire_old_soup Changed\n");
        ret = 1;
    }
    if(last->valuemode_Firewood != cur->valuemode_Firewood)
    {
        GIZWITS_LOG("valuemode_Firewood Changed\n");
        ret = 1;
    }
    if(last->valuemode_Steamed_bag != cur->valuemode_Steamed_bag)
    {
        GIZWITS_LOG("valuemode_Steamed_bag Changed\n");
        ret = 1;
    }
    if(last->valuedianji_one != cur->valuedianji_one)
    {
        GIZWITS_LOG("valuedianji_one Changed\n");
        ret = 1;
    }
    if(last->valueset_the_time != cur->valueset_the_time)
    {
        GIZWITS_LOG("valueset_the_time Changed\n");
        ret = 1;
    }
    ...
    if(last->valuetime_left != cur->valuetime_left)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuetime_left Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(last->valueyanwu != cur->valueyanwu)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueyanwu Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(last->valuehuoyan != cur->valuehuoyan)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuehuoyan Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(submit_BYTEOFFSET,submit_BITOFFSET,submit_LEN,(uint8_t *)devStatusPtr,dataPoints->valuesubmit);
    gizStandardCompressValue(ignition_BYTEOFFSET,ignition_BITOFFSET,ignition_LEN,(uint8_t *)devStatusPtr,dataPoints->valueignition);
    gizStandardCompressValue(mode_Slow_fire_old_soup_BYTEOFFSET,mode_Slow_fire_old_soup_BITOFFSET,mode_Slow_fire_old_soup_LEN,(uint8_t *)devStatusPtr,dataPoints->valuemode_Slow_fire_old_soup);
    gizStandardCompressValue(mode_Firewood_BYTEOFFSET,mode_Firewood_BITOFFSET,mode_Firewood_LEN,(uint8_t *)devStatusPtr,dataPoints->valuemode_Firewood);
    gizStandardCompressValue(mode_Steamed_bag_BYTEOFFSET,mode_Steamed_bag_BITOFFSET,mode_Steamed_bag_LEN,(uint8_t *)devStatusPtr,dataPoints->valuemode_Steamed_bag);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valuedianji_one = gizY2X(dianji_one_RATIO,  dianji_one_ADDITION, dataPoints->valuedianji_one); 
    devStatusPtr->valueset_the_time = gizY2X(set_the_time_RATIO,  set_the_time_ADDITION, dataPoints->valueset_the_time); 
    ...
    devStatusPtr->valuetime_left = exchangeBytes(gizY2X(time_left_RATIO,  time_left_ADDITION, dataPoints->valuetime_left)); 
    devStatusPtr->valueyanwu = exchangeBytes(gizY2X(yanwu_RATIO,  yanwu_ADDITION, dataPoints->valueyanwu)); 
    devStatusPtr->valuehuoyan = exchangeBytes(gizY2X(huoyan_RATIO,  huoyan_ADDITION, dataPoints->valuehuoyan)); 
